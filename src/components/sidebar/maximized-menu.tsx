...
const MaxMenu = ({
  ...
  const toggleBtnCls = cn(
    'p-1.5 md:p-2 rounded-lg transition-all duration-200 cursor-pointer',
  ...
  return (
    <div className="py-2 px-3 md:py-3 md:px-4 flex flex-col h-full">
      <div className="flex justify-between items-center mb-3 md:mb-6">
        ...
        <button
          onClick={onExpand}
          className={cn(toggleBtnCls, 'hidden md:flex animate-fade-in opacity-0 delay-300 fill-mode-forwards')}
        >
          <Menu className="w-5 h-5" />
        </button>
        <button
          onClick={onMobileClose}
          className={cn(toggleBtnCls, 'md:hidden')}
        >
          <X className="w-5 h-5" />
        </button>
      </div>
      ...
      <div className="animate-fade-in opacity-0 delay-300 fill-mode-forwards flex flex-col justify-between flex-1 overflow-y-auto">
        <div className="flex flex-col">
          <p className="text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wider mb-1.5 md:mb-2">
            Menu
          </p>
          <div className="flex flex-col mb-1 md:mb-2">
            ...
            {showOnboarding && (
              ...
              >
                ...
                {stepsCompleted === 4 ? (
                  <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-[var(--status-success)] text-white text-[10px] font-bold flex-shrink-0">
                    ✓
                  </span>
                ) : (
                  <span
        ...
        {personaItems.length > 0 && (
          <div className="mx-0 mt-2">
            <div className="rounded-xl border border-[var(--border-default)] bg-[var(--bg-hover)] p-2">
                  ...
                  return (
                    <Link
                      ...
                      className={cn(
                        ...
                        isActive
                          ? 'bg-[var(--bg-active)] border border-[var(--border-accent)]'
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0 ml-0">
                        ...
                        <p className="text-[10px] text-[var(--text-muted)] truncate">{item.domainName}</p>
                      </div>
                    </Link>
                  )
                })}
        ...
        <div className="flex flex-col border-t border-[var(--border-default)] pt-2 md:pt-3 mt-2 md:mt-3">
          <p className="text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wider mb-1.5 md:mb-2">
            Options
          </p>
          <button
