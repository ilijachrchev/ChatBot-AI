...
const TeaserBubble = ({ message, position, onDismiss }: Props) => {
  return (
    ...
    >
      <div className="relative bg-white border border-[var(--border-default)] rounded-2xl px-4 py-3 shadow-lg shadow-slate-200/50 max-w-[200px] min-w-[140px]">
        ...
        <button
          ...
          className="absolute top-2 right-2 w-4 h-4 rounded-full flex items-center justify-center text-[var(--text-muted)] hover:text-[var(--text-secondary)] hover:bg-slate-100 transition-colors cursor-pointer"
        >
          <X className="w-3 h-3" />
        </button>
