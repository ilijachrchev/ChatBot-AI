...
export async function sendLiveChatNotification({
  ...
  const truncatedMessage = lastMessage.length > 120 ? lastMessage.slice(0, 120) + '…' : lastMessage
  ...
  try {
    const { data, error } = await resend.emails.send({
      from: 'SendWise AI <notifications@sendwiseai.com>',
      to: [ownerEmail],
      subject: `🔔 Live chat requested on ${domainName}`,
      html: `
        <!DOCTYPE html>
        <html>
          <head>
            ...
            <title>Live Chat Requested</title>
          </head>
          <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
            <div style="background: linear-gradient(135deg, #3b82f6 0%, #8b5cf6 100%); padding: 30px; text-align: center; border-radius: 10px 10px 0 0;">
              <h1 style="color: white; margin: 0; font-size: 28px;">Live Chat Requested 💬</h1>
            </div>

            <div style="background: white; padding: 30px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 10px 10px;">
              <p style="font-size: 16px; margin-top: 0;">Hi ${ownerName},</p>

              <p style="font-size: 16px;">A customer on <strong>${domainName}</strong> has requested to speak with a human agent.</p>

              <div style="background: #eff6ff; border-left: 4px solid #3b82f6; padding: 15px; margin: 25px 0; border-radius: 0 6px 6px 0;">
                <table style="width: 100%; border-collapse: collapse;">
                  <tr>
                    <td style="padding: 6px 0; color: #6b7280; width: 130px;">Domain:</td>
                    <td style="padding: 6px 0; font-weight: 600;">${domainName}</td>
                  </tr>
                  <tr>
                    <td style="padding: 6px 0; color: #6b7280;">Customer:</td>
                    <td style="padding: 6px 0; font-weight: 600;">${customerEmail ?? 'Anonymous visitor'}</td>
                  </tr>
                  <tr>
                    <td style="padding: 6px 0; color: #6b7280; vertical-align: top;">Last message:</td>
                    <td style="padding: 6px 0; font-style: italic;">"${truncatedMessage}"</td>
                  </tr>
                  <tr>
              ...
              <div style="text-align: center; margin: 30px 0;">
                <a href="https://sendwiseai.com/conversation" style="display: inline-block; background: linear-gradient(135deg, #3b82f6 0%, #8b5cf6 100%); color: white; padding: 12px 30px; text-decoration: none; border-radius: 6px; font-weight: 600;">Open Conversation →</a>
              </div>

              <hr style="border: none; border-top: 1px solid #e5e7eb; margin: 25px 0;">

              <p style="font-size: 12px; color: #9ca3af; margin-bottom: 0;">You're receiving this because live chat notifications are enabled for ${domainName}. Manage in Domain Settings → Realtime &amp; Handoff.</p>
            </div>
          </body>
        </html>
      `,
    ...
    if (error) {
      console.error('Error sending live chat notification:', error)
