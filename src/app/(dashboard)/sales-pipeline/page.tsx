...
const STAGES = [
  { key: 'NEW', label: 'New', color: 'border-slate-300 dark:border-slate-700', headerColor: 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300' },
  { key: 'CONTACTED', label: 'Contacted', color: 'border-blue-300 dark:border-blue-800', headerColor: 'bg-blue-50 dark:bg-blue-950/50 text-blue-700 dark:text-blue-300' },
  ...
  if (!domainId) {
    return (
      <div className="p-6 md:p-8 max-w-2xl mx-auto">
        ...
        <p className="text-slate-500 dark:text-slate-400 mb-8">Select a domain to view its pipeline.</p>
        <div className="flex flex-col gap-3">
          {salesDomains.map(d => (
            <Link
              key={d.id}
              href={`/sales-pipeline?domain=${d.id}`}
              className="flex items-center justify-between p-4 rounded-xl border-2 border-slate-200 dark:border-slate-800 hover:border-slate-400 dark:hover:border-slate-600 bg-white dark:bg-slate-950 transition-all"
            >
          ...
          {salesDomains.length === 0 && (
            <p className="text-slate-400 dark:text-slate-500 text-sm">No domains with Sales Agent persona found.</p>
          )}
        </div>
      </div>
    )
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-8">
        <div>
          ...
          <div className="flex items-center gap-3 mt-1">
            ...
            <span className="text-sm text-slate-500 dark:text-slate-400">{customers.length} leads total</span>
          </div>
        </div>
        ...
        {STAGES.map(stage => {
          ...
          return (
            <div key={stage.key} className="flex-shrink-0 w-72">
              ...
              <div className={cn('rounded-xl border-2 min-h-[200px] p-2 flex flex-col gap-2', stage.color, 'bg-slate-50/50 dark:bg-slate-900/30')}>
                {leads.length === 0 && (
                  <div className="flex-1 flex items-center justify-center border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-lg p-6">
                    <span className="text-xs text-slate-400 dark:text-slate-600 text-center">No leads in this stage</span>
                  </div>
                )}
                {leads.map(lead => {
                  ...
                  return (
                    <div
                      key={lead.id}
                      className="bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 p-3 flex flex-col gap-1.5 hover:shadow-md transition-shadow"
                    >
                      <div className="flex items-start justify-between gap-2">
                        <span className="text-sm font-semibold text-slate-900 dark:text-white truncate">
                          {lead.email ?? 'Anonymous'}
                        </span>
                        <GripVertical className="h-4 w-4 text-slate-300 dark:text-slate-700 flex-shrink-0" />
                      </div>
                      {lastMsg && (
